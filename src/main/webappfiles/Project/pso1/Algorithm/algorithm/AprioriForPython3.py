...
def writeResults(items, rules):
    with open("E:\\Apriori\\result.txt", 'w', 'utf-8') as wf:
        for item, support in sorted(items, key=lambda x: x[1], reverse=True):
            wf.write("item: %s , %.3f" % (str(item), support) + '\n')
        print("\n------------------------ RULES:")
        for rule, confidence in sorted(rules, key=lambda x: x[1], reverse=True):
            pre, post = rule
            wf.write("Rule: %s ==> %s , %.3f" % (str(pre), str(post), confidence) + '\n')


def dataFromFile(fname):
        """Function which reads from the file and yields a generator"""
        csvfile = codecs.open(fname, 'rU', 'utf-8')

...
.run(['taRegisterTool', '$window', '$timeout', 'AvatarUploadService', 'taTranslations', 'taSelection', 'taToolFunctions', '$sanitize', 'taOptions', '$log',
    function(taRegisterTool, $window, $timeout, AvatarUploadService, taTranslations, taSelection, taToolFunctions, $sanitize, taOptions, $log){
    ...
    taRegisterTool('insertImage', {
        ...
        action: function(deferred){
            ...
            AvatarUploadService.open("ArticleImage", {compress:true, width:1000,preview:false},
                function (result) {
                    if(result){
                        rangy.restoreSelection(sel);
                        //sel.collapseToEnd();
                        var embed = '<img src="http://omhemfx8a.bkt.clouddn.com/' + result + '">';
                        deferred.resolve(vm.$editor().wrapSelection('insertHTML', embed, true));
                    }
                }, function () {});
            return false;
        },
